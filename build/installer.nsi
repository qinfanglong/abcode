...
Section "MainSection" SEC_MAIN
    SetOutPath "$INSTDIR"
    ; PyInstaller 单文件 exe 已内置 frontend/backend，无需复制源码副本
    File "${ROOT}\dist\ABcode.exe"
    File "${ROOT}\build\icon.ico"
    ; 生成卸载程序（此前缺失，导致卸载快捷方式指向不存在的 uninstall.exe）
...
; Uninstaller
Section "Uninstall"
    ; 先结束 ABcode 进程，避免正在运行的 exe/数据库文件被锁定无法删除
    ExecWait 'taskkill /F /IM ${APP_EXE}'
    ; 递归删除整个安装目录（含 data/ 用户数据），保证卸载后无残留
    RMDir /r "$INSTDIR"
