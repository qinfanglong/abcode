...
def _extract_json(text):
    ...
    容忍：```json 代码块、前后说明文字、多余空白。提取失败返回 None。
    """
...
{{
  "subtasks": [
    ...
    ...
        ...
        plan = _extract_json(plan_result.get("output"))
        subtasks = (plan or {}).get("subtasks") if isinstance(plan, dict) else None
        if not isinstance(subtasks, list) or not subtasks:
            # 模型未按 JSON 返回（常见：输出说明文字/markdown）时回退为顺序执行，保证有输出
            return self._run_sequential(task, context)
