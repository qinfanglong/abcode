...
# 知识库目录：源码模式在仓库根 data/kb；frozen 模式在 exe 旁 data/kb（可持久）
if getattr(sys, 'frozen', False):
    _KB_ROOT = Path(sys.executable).parent / "data" / "kb"
else:
    _KB_ROOT = Path(__file__).parent.parent / "data" / "kb"

KB_DIR = Path(os.environ.get("ABCODE_KB", _KB_ROOT))
