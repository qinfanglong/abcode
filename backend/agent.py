    ...
    try:
        if name == "get_current_time":
            ...
            try:
                ...
                results = rag.search(query, top_k=top_k, kb_id=args.get("kb_id") or None)
            except Exception as e:
                return False, f"知识库检索失败: {e}"
            if not results:
                return True, "知识库未检索到相关内容"
            lines = []
            for i, r in enumerate(results):
                lines.append(f"[{i+1}] 《{r.get('doc_name', '')}》 相关度{r.get('score', 0)}\n{r.get('content', '')}")
            return True, "\n\n".join(lines)
