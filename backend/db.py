...
# 数据目录：源码模式在仓库根 data/；PyInstaller(frozen) 模式在 exe 旁的 data/（可持久，不落 %TEMP%）
if getattr(sys, 'frozen', False):
    _DEFAULT_DB = Path(sys.executable).parent / "data" / "abcode.db"
else:
    _DEFAULT_DB = Path(__file__).parent.parent / "data" / "abcode.db"

DB_PATH = Path(os.environ.get("ABCODE_DB", _DEFAULT_DB))
...
def init_db():
    ...
    -- 工作流模板（表定义见 init_workflows_table；此处旧版 schema 已废弃，不再创建，
    -- 否则全新数据库会先建出缺少 nodes/edges 列的旧表，导致模板初始化报错）
    -- CREATE TABLE IF NOT EXISTS workflow_templates (...) -- 201: 旧定义已删除
    -- 工作流测试用例
    ...
    try:
        ...
        if "workflow_id" not in ct_cols:
            conn.execute("ALTER TABLE conv_tools ADD COLUMN workflow_id TEXT DEFAULT ''")
    except Exception:
        pass
    conn.commit()
    conn.close()
...
def get_conv_tools(conv_id):
    ...
    conn.close()
    if not row:
        return {"skill_ids": [], "mcp_ids": [], "connector_ids": [], "expert_id": "", "workflow_id": ""}
    d = dict(row)
    ...
    d["workflow_id"] = d.get("workflow_id", "") or ""
    return d


def set_conv_tools(conv_id, skill_ids=None, mcp_ids=None, connector_ids=None, expert_id=None, workflow_id=None):
    ...
    if workflow_id is not None:
        cur["workflow_id"] = workflow_id
    conn = get_conn()
    conn.execute("""INSERT INTO conv_tools (conv_id,skill_ids,mcp_ids,connector_ids,expert_id,workflow_id) VALUES (?,?,?,?,?,?)
                    ON CONFLICT(conv_id) DO UPDATE SET
                      skill_ids=excluded.skill_ids, mcp_ids=excluded.mcp_ids, connector_ids=excluded.connector_ids, expert_id=excluded.expert_id, workflow_id=excluded.workflow_id""",
                 (conv_id, json.dumps(cur["skill_ids"]), json.dumps(cur["mcp_ids"]), json.dumps(cur["connector_ids"]), cur["expert_id"], cur["workflow_id"]))
    conn.commit()
    conn.close()
...
def init_workflows_table():
    """初始化工作流相关表"""
    conn = get_conn()
    # 迁移：老版本 workflow_templates 使用旧 schema（workflow_json/tags/is_builtin，
    # 无 nodes/edges 列）。全新数据库若先被旧代码建表，这里必须识别并重建，
    # 否则模板初始化 INSERT nodes 列会报 "table workflow_templates has no column named nodes"
    try:
        cols = [r[1] for r in conn.execute("PRAGMA table_info(workflow_templates)").fetchall()]
        if cols and "nodes" not in cols:
            conn.execute("DROP TABLE workflow_templates")
            print("[db] 迁移：重建 workflow_templates（旧 schema -> 新 schema）")
    except Exception as e:
        print(f"[db] 迁移 workflow_templates 跳过: {e}")
    conn.executescript("""
    -- 工作流定义
    CREATE TABLE IF NOT EXISTS workflows (
        id TEXT PRIMARY KEY,
